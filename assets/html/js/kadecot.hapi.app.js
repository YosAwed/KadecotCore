...
外部アプリのkadecot.initの中からreqMyPageConnectedが
呼ばれるのを待って、onMyPageConnectedを返す。

onMyPageConnectedは、その前提として、外部アプリからのonGetSetValue呼び出しに対応
できるようにしておく必要がある。
...
running {
	manifest : the manifest of running app
	,win : アプリのwindow
	,origin : origin
	,cleanup : function   //終了。runningのクリーニングやら、全てのポーリングの停止やらする。引数がtrueの場合はウィンドウを閉じる。
}
...
varNameMap : アクセスオブジェクト中のデバイス名とアクセス変数の名前をキーとして、Kadecot側の
	名前のペアを配列で
	varNameMap[accessDevName][accessVarName] = [device nickname ,device property name , option string if exist]
...
var myPageURL = (location.href.indexOf('?')!=-1
...
function getTempFromECHONETLite2ByteArray(b){
  ...
  return  parseFloat(  (( tempInt > 0x7fff ) ? (tempInt - 0xf554-2732)*0.1 : tempInt * 0.1).toFixed(1)  ) ;
}
			...
			function(manifests){
				...
				if( !kHAPI.isOnAndroid )
					addEventListener('message'
						,function(e){kHAPI.app.onMsgFromApp(e.origin,JSON.parse(e.data));},false) ;
			...
			kHAPI.app._manifests = {data:[
				...
				,{
					...
					,"devices":[
						...
						,{ "name" : "ExtTemp", "protocol" : "ECHONET Lite", "deviceType" : "0x0011", "description":"外気温のセンサー。"
						   ,"access":{"0xe0":{"name":"val","polling":600,"option":"echoTemperature"}}
						 }
					]
				}
			]}.data;
		...
		function postMain(){
			...
			for( var di=0;di<ds.length;++di ){
				(function(){
					...
					if( rd === undefined ){
						...
						if( rd.length==0 ) rd = undefined ;
						else {rd=rd[0];d.assignedDevName = rd.nickname ;}
					}
					...
					for( var prop in d.access ){
						(function(){
							...
							manif.varNameMap[ d.name ][d_access_prop.name] =
								(option === undefined
							...
							getcalls.push( function(){
								...
								kHAPI.get( [rd.nickname,_prop] , function(result,success){
									...
									if( typeof varname !== 'string' ){--get_total; return ;}
							...
							if( typeof d_access_prop.polling === 'number' ){
								...
								stopPollFuncs.push( function(){poll_stopper=true;} ) ;
								...
								var poll_func = function(){
									...
									kHAPI.get( [rd.nickname , _prop] , function(result,success){
										if(success){
											...
											kHAPI.app.postMsgToApp( 'onPropertyChanged',[d.name,varName,result] ) ;
										}
			...
			this.running.onPropertyChanged = function(info){
				...
				for( var di=0;di<ds.length;++di ){
					...
					if( d.access[propname] === undefined )
					...
					for( var pi=0;pi<info.property.length;++pi ){
						...
						kHAPI.app.postMsgToApp(
							'onPropertyChanged'
							,[d.name,d.access[propname].name,newval] ) ;
					}
					break ;
				}
			} ;
			...
			if( get_total === 0 )	kHAPI.app.postMsgToApp( 'onMyPageConnected',[ao] ) ;
			else for( var gi=0;gi<getcalls.length;++gi )	getcalls[gi]() ;
		} ;
