...
function l(val, msg) {
  console.log((msg === undefined ? 'none' : msg) + ' : ' + JSON.stringify(val));
};
...
var kHAPI = {
  ...
  init: function(oncomplete_func) {
    ...
    function init_real() {
      ...
      // deviceAccessQueue["nickname"] = undefined (if there is not previous
      // gets)
      // deviceAccessQueue["nickname"] =
      // [{props:[prop1_1,prop1_2,,,],callback:function(){}},
        ...
        if (deviceAccessQueue[nickname] === undefined) {
          ...
          setTimeout(function() {
            ...
            for (var i = 0; i < props_and_callbacks.length; i++) {
              // equal to send_args =
              // send_args.concat(props_and_callbacks[i].props);
              ...
              start_and_end_index.push({
                f: send_args.length - 1 - props_and_callbacks[i].props.length,
      ...
      if (kHAPI.isOnAndroid) {
        ...
        wrapFuncs = wrapFuncs.concat(['fullInitialize',
            'enableWebSocketServer', 'enableServerNetwork']);
      } else {
        // ///////////////////////
      ...
      kHAPI.local.init();
      if (!kHAPI.isOnAndroid) Local = kHAPI.local;
    ...
    /*
     * // Pre-include js required files. var incfiles = [
     * 'net','dev','app','local'] ; var incfilenum = incfiles.length ;
     * $.ajaxSetup({async: false}); for( var i=0;i<incfiles.length;++i ){
     * $.getScript('js/kadecot.hapi.'+incfiles[i] + '.js' ,function(){
     * if(--incfilenum === 0 ){init_real.call(kHAPI); }} ); }
