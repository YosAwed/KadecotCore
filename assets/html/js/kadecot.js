...
　マイページに接続したことがなければ、ユーザーがマイページ上でアプリ認証を行う必要が
　あります。

　kadecot.initは二つの引数を取ります。どちらも省略不可です。
　一つ目はmanifestオブジェクトです。manifestオブジェクトはクライアントアプリが
　ホームサーバーに対して要求する機能の定義や、kadecot内に表示されるアプリの
　説明文などを設定するものです。
　二つ目の引数は初期化完了時に呼ばれるコールバック関数です。このコールバックが呼ばれる
　ときには引数としてデバイスにアクセスするためのオブジェクト（アクセスオブジェクト）が
　一つ渡されてきます。このオブジェクトの内容については、manifestオブジェクトの仕様を説明した
　上で説明します。

  ○manifestオブジェクト

　kadecot.init()の最初の引数として与えられるmanifestオブジェクトは以下の情報を含んでいます。
　　+クライアントアプリそのものの説明
　　+クライアントアプリがアクセスする機器の種類と、その中で関心があるプロパティ（機能名）
　　　の情報。さらに、そのプロパティの値をどういう名前の変数でアクセスするか、何秒周期で
　　　アップデートするかの情報。

　　例えば、次のようになります。

	{"title":"かっこいいアプリ"
		,"subtitle":"かっこよさに気を失いそうなアプリです。"
		,"description":"かっこよさとは何かについて、まじめに考えてみたアプリです。"
		,"icon":"./icon.png"
		,"devices":[
			{ "name" : "Aircon", "protocol" : "ECHONET Lite", "deviceType" : "0x0130", "description":"エアコン"
				,"access":{
					"0x80":{"name":"Power","polling":15}
					,"0xb0":{"name":"Mode","polling":16}
					,"0xb3":{"name":"Temp","polling":17}
					}
			}
			,{ "name" : "Light", "protocol" : "ECHONET Lite", "deviceType" : "0x0290", "description":"室内の照明"
...
　　title : アプリのタイトルです。特に制限はありませんが、アプリアイコンの下に表示
　　　　　　される関係で、8文字以内推奨です。全てのタグは削除されます。
　　subtitle : アプリを一行で説明する文です。こちらは、アイコンを選択状態にした時にトーストで
　　　　　　表示される文です。全てのタグは削除されます。
　　description : アプリの設定画面を開いた時に表示されるもので、こちらは長さに制限はありません。
　　　　　　<image>以外のタグは削除されます。

　　icon : アイコン画像のURLです。64x64にしてください。絶対パスでもいいし、相対パスの場合は
　　　　　このクライアントアプリがあるURLからの相対になります

　　devices : このアプリが操作対象とする機器を表しています。上の例では、エアコンと室内の
　　　　　　照明を操作対象にしていることがわかります。
...
　　"0x80"というメンバを含んでいました。その部分を抜き出してみます。
	,{ "name" : "Light", "protocol" : "ECHONET Lite", "deviceType" : "0x0290", "description":"室内の照明"
	   	  ,"access":{"0x80":{"name":"Power","polling":10}}
	 }
...
　　とすることで、照明をONすることができます。
　　0x30というのは、プロトコル上規定された値です。ECHONET Liteでは、EDTと呼ばれている16進数に
　　なります。

　　では、具体的に今回ao.Lightの中にどのようなものが作られているのか見てみましょう。

　　ao.Light = {
　　　　active :      value　　 //  true | false | undefined　現在の機器の状態。代入不可。

　　　　,Power :        value　　 //  電源状態の参照や代入のための変数。ただし、参照の場合、その値はポーリング等
　　　　　　　　　　　　　　　　　//　によって得られた最後の値。

　　　　,setPower : function(newval, onsuccessfunc, bCallChangePower)
　　　　　　　　　　　　　　　    // Powerへの代入と同じく、電源の値をセットするための関数だが、成功時の
　　　　　　　　　　　　　　　　　// コールバック関数を設定できる。一つ目の引数としてセットする値、二つ目として
　　　　　　　　　　　　　　　　　// セットに成功したときに呼ばれるコールバック関数、三つ目として、この関数の
				  // 結果値が変化したときに、onChangePower(後述)を呼び出すかどうかの真理値
				  // (省略可。default=false)


　　　　,getPower : function(onsuccessfunc, bCallChangePower)
　　　　　　　　　　　　　　　　　// 現在のリアルタイムの電源の値をサーバーから得るための関数。
　　　　　　　　　　　　　　　　　// 引数として、得られた後のコールバック関数、この関数の結果値が
　　　　　　　　　　　　　　　　　// 変化したときに、onChangePower(後述)を呼び出すかどうかの真理値
　　　　　　　　　　　　　　　　　// (省略可。default=false)
　　} ;
...
　　このように、Lightの中にはPowerというプロパティだけでなく、activeという変数と、(set|get)Powerという関数が
　　追加されています。
...
　　設定や取得ができます。ただし、アクセス変数の使い方にはいくつか留意して頂きたいことがあります。
　　　1. アクセス変数に代入しても、その直後に参照して得られる値は古いままです。なぜかというと、代入により
　　　　セットされる制御情報がネットを通じて機器に伝達されるまでには時間がかかりますし、途中で何かエラーが起こる
　　　　可能性もあるからです。成功したときにのみ、参照で得られる値の更新がされます。
　　　2. アクセス変数は、基本的にはポーリングによって値が更新されるので、必ずしも実際の機器の最新の状態と一致
　　　　しないことがあります（ポーリング周期は、manifestオブジェクトのpollingプロパティで設定します（秒単位）。
　　　　ただし、プロトコル上値が変化したことが自動的に通知されるような場合に限り、これが常に最新の値であることを
　　　　信じることができます。（ECHONET Liteで状変通知が必須となっているプロパティはそうなります）
　　　3. センサーの値など、取得はできても代入はできないプロパティもあります。この場合はいくら代入しても値が
　　　　変化することはありません。
...
　　出されます。アクセス変数の値が変更されるのは、以下の4通りのケースがあります。
　　　1. ポーリングの結果変化していた時
　　　2. 状変通知があった時
　　　3. getPower呼び出しにより最新の値を取得したら前と違う値だったとき
　　　4. setPower呼び出しにより、プログラムから値を変更したとき
...
　　機器オブジェクトが作られ、さらにaccessを設定しておくと、そのnameメンバの名前を持ったアクセス変数が作られ、
　　get*, set*、それに、必要に応じてonChange*というコールバックを設定できることがわかりました。


    さらにもう一つ、アプリが終了したことを通知するstop()という関数があります。
　　これは、ポーリングの終了、その他クリーンナップを行います。もう機器操作を使わないときに
　　引数なしで呼び出すとその後のパフォーマンスが向上します。
　　また、引数にtrueを入れると、アプリウィンドウが閉じる処理を行います。アプリのonunloadの中などで強制的に
　　引数trueで呼び出していただけると助かります。絶対呼び出さなければいけないというものでもありませんが、
　　終了後の無駄なパフォーマンス低下を抑制できます。
　　一度終了した後の再開は再度kadecot.initを呼び出すことにより可能…（だと思われますが、あまりちゃんとチェック
　　していません）
　　
...
　　　　,Mode	 : object
　　　　,getMode : function(onsuccessfunc, bCallChangePower)
　　　　,setMode : function(newval, onsuccessfunc, bCallChangePower)

　　　　,Temp	 : object
　　　　,getTemp : function(onsuccessfunc, bCallChangePower)
　　　　,setTemp : function(newval, onsuccessfunc, bCallChangePower)
　　　}
　　　,Light : {
　　　　active : boolean
...
　　説明はここまでになります。
　　今回説明しきれなかったものとして、manifestのaccessの各プロパティ定義中に書ける
　　optionという設定があります。これは、例えばECHONET Liteの温度センサーは2バイトの
　　配列で温度を表現していますが、これを浮動小数値一つで扱えるようにするためのもの
　　です。その他、プロトコルで流れてくる値そのままを使うとわかりづらいものを、より
　　直観に即した操作ができるように変換します。
