...
$(document).on("pagecreate",function(){
  ...
  kHAPI.init(function(){
    ...
    kHAPI.devListHandlers.onDeviceDeleted = function(newdevice,newlist){
      return kHAPI.devListHandlers.onUpdateList(newlist);
    };

    kHAPI.onPropertyChanged = function(prop_change){
     // console.log(prop_change);
      update_one_epc(prop_change.nickname,prop_change.property[0].name,
...
var addDeviceForGoodPlace = function(newdev){
  ...
  kHAPI.dev.sortDevices();
  devs = devs.filter(function(d){return !isControler(d);});

  for(var i=0;i<devs.length;i++){
    if(devs[i].nickname === newdev.nickname){
      if(i != 0){
        $("#"+getNicknameHash(devs[i-1].nickname)).after(makeDeviceLi(newdev));
      }else{
...
var onNotifyServerSettingsOnAndroid = function(settings){
  ...
  if(initialNotifyServerSettings &&
     ...
     !settings.network.isDeviceAccessible){
      setTimeout(function(){return onClickRegisterButton(true);},500);
  }
  ...
  if(device.protocol === 'ECHONET Lite'){
    ...
    $("#get_property_button").click(function(){
      ...
      myget([nickname,"0x9f"],function(data,success){
        if(!success || data.property === undefined) {
          return;
        }
...
var deviceRemocons = {
  ...
  "0x0260":[{epc:"0xe0",edt:[0x41],text:"Open"},
            {epc:"0xe0",edt:[0x42],text:"Close"}],
  // Water heater
  "0x026b":[{epc:"0xb0",edt:[0x41],text:"Auto boil"},
            {epc:"0xb0",edt:[0x42],text:"Manual boil"},
            {epc:"0xb0",edt:[0x43],text:"Manual boil stop"},
            {epc:"0xd1",edt:[40],text:"Supply 40℃"},
            {epc:"0xd1",edt:[42],text:"Supply 42℃"},
            {epc:"0xd1",edt:[44],text:"Supply 44℃"},
            {epc:"0xd3",edt:[38],text:"Bath 38℃"},
            {epc:"0xd3",edt:[40],text:"Bath 40℃"},
            {epc:"0xd3",edt:[42],text:"Bath 42℃"},
            {epc:"0xe3",edt:[0x41],text:"Bath water auto"},
            {epc:"0xe3",edt:[0x42],text:"Bath water manual"},
            {epc:"0xe4",edt:[0x41],text:"Heat refill on"},
            {epc:"0xe4",edt:[0x42],text:"Heat refill off"},
            {epc:"0xe6",edt:[0x41],text:"Warm on"},
            {epc:"0xe6",edt:[0x42],text:"Warm off"}
           ]
};
...
//		+'<input type="range" name="slider" id="blindAngleSlider" min="0" max="180" onchange="console.log(this.value); " />' ;
...
var update_map = function(nickname){
  ...
  var update_other_sensors = function(){
    ...
    ret['0xb1'] = function(newval){
        status_txt_area.html('  '+(newval[0]==0x41?'<font color="#c00">ON<font>'
                                   :'OFF'));
    };
    return ret;
  };
  ...
  if( d.protocol === 'ECHONET Lite' ){
    ...
    } else if( dtNum == 0x0262 || dtNum == 0x0260 ){   // Curtain or ElectricallyOperatedShade
...
var myget = function(args,callback,with_out_queue){
  ...
  if(dev === undefined || dev === null || !dev.active){
    return;
  }
...
var areYouSure = function(big,small,yestext,notext,yescallback,nocallback){
  ...
  $.mobile.changePage("#sure",{transition:"none"});
};
