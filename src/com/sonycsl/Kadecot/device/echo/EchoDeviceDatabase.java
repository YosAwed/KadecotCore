...
public class EchoDeviceDatabase {
	...
	private EchoDeviceDatabase(Context context) {
		...
		columns.put(KEY_ID, DatabaseOpenHelper.DATA_TYPE_INTEGER+DatabaseOpenHelper.OPTION_PRIMARY_KEY_AUTOINCREMENT);
		columns.put(KEY_DEVICE_ID, DatabaseOpenHelper.DATA_TYPE_INTEGER+DatabaseOpenHelper.OPTION_NOT_NULL);
		columns.put(KEY_ADDRESS, DatabaseOpenHelper.DATA_TYPE_TEXT+DatabaseOpenHelper.OPTION_NOT_NULL);
		columns.put(KEY_ECHO_CLASS_CODE, DatabaseOpenHelper.DATA_TYPE_INTEGER+DatabaseOpenHelper.OPTION_NOT_NULL);
		columns.put(KEY_INSTANCE_CODE, DatabaseOpenHelper.DATA_TYPE_INTEGER+DatabaseOpenHelper.OPTION_NOT_NULL);
	...
	public EchoDeviceData getDeviceData(EchoObject device) {
		...
		return getDeviceData(address, device.getEchoClassCode() & 0xFFFF, device.getInstanceCode() & 0xFF);
	}

	public EchoDeviceData getDeviceData(String address, int echoClassCode, int instanceCode) {
		Cursor c = mHelper.getCursor(new DatabaseOpenHelper.Where(
				new String[]{KEY_ADDRESS, KEY_ECHO_CLASS_CODE, KEY_INSTANCE_CODE}
				, new String[]{address, Integer.toString(echoClassCode), Integer.toString(instanceCode)}));
	...
	public List<Integer> getLocalDeviceInstanceCodeList(short echoClassCode) {
		...
		Cursor cursor = mHelper.getCursor(
				new DatabaseOpenHelper.Where(
						new String[]{ KEY_ADDRESS, KEY_ECHO_CLASS_CODE}
						, new String[]{LOCAL_ADDRESS, Integer.toString(echoClassCode & 0xFFFF)})
	...
	public class EchoDeviceDatabaseHelper extends DatabaseOpenHelper {
		...
		public EchoDeviceDatabaseHelper(Context context, String dbName,
