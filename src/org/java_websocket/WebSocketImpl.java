    ...
    private boolean decodeHandshake(ByteBuffer socketBufferNew) {
        ...
        if (tmpHandshakeBytes == null) {
            ...
            if (tmpHandshakeBytes.remaining() < socketBufferNew.remaining()) {
                ByteBuffer buf = ByteBuffer.allocate(tmpHandshakeBytes.capacity()
                        + socketBufferNew.remaining());
                    ...
                    if (draft == null) {
                        for (Draft d : knownDrafts) {
                            ...
                            try {
                                ...
                                String res = "";
                                Iterator<String> ite = handshake.iterateHttpFields();
                                while (ite.hasNext()) {
                                    String value = ite.next();
                                    res += ", " + value + "= " + handshake.getFieldValue(value);
                                }
                                Log.e(TAG,
                                ...
                                if (handshakestate == HandshakeState.MATCHED) {
                                    ...
                                    res = "";
                                    ite = response.iterateHttpFields();
                                    while (ite.hasNext()) {
                                        String value = ite.next();
                                        res += ", " + value + "= " + response.getFieldValue(value);
                                    }
                                    Log.e(TAG,
                                            "Server response: httpStatus="
