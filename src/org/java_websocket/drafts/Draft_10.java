...
public class Draft_10 extends Draft {
    ...
    public ByteBuffer createBinaryFrame(Framedata framedata) {
        ...
        ByteBuffer buf =
            ByteBuffer.allocate(1 + (sizebytes > 1 ? sizebytes + 1 : sizebytes) + (mask ? 4 : 0)
                + mes.remaining());
    ...
    @Override
    public List<Framedata> translateFrame(ByteBuffer buffer) throws LimitExedeedException,
        ...
        if (incompleteframe != null) {
            ...
            while (true) {
                try {
                    buffer.mark();
                    int available_next_byte_count = buffer.remaining();// The number of bytes
                                                                       // received
                    int expected_next_byte_count = incompleteframe.remaining();// The number of
                                                                               // bytes to complete
                                                                               // the incomplete
                                                                               // frame
                    ...
                    incompleteframe
                        .put(buffer.array(), buffer.position(), expected_next_byte_count);
    ...
    public Framedata translateSingleFrame(ByteBuffer buffer) throws IncompleteException,
        ...
        if (rsv != 0) throw new InvalidFrameException("bad rsv " + rsv);
        ...
        if (MASK) {
            ...
            for (int i = 0; i < payloadlength; i++) {
                payload
                    .put((byte)((byte)buffer.get( /* payloadstart + i */) ^ (byte)maskskey[i % 4]));
            }
        } else {
